...
def make_cfg_1ch(cfg, thermal_mean: float = 128.0, thermal_std: float = 58.0) -> None:
    """
    Adjust Detectron2 normalization for 1 channel (thermal-only).
    """
    cfg.MODEL.PIXEL_MEAN = [float(thermal_mean)]
    cfg.MODEL.PIXEL_STD  = [float(thermal_std)]


def patch_first_conv_to_1ch(model: nn.Module) -> None:
    """
    Replace the FIRST Conv2d expecting 3 input channels with an equivalent 1-channel layer.
    The single channel is initialized as the mean across the original 3 input weights.
    No-op if already patched.
    """
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and module.in_channels == 3:
            ...
            with torch.no_grad():
                w = module.weight  # [out, 3, k, k]
                # average across the input channel dim to produce a single-channel kernel
...
def ensure_model_pixel_stats_1ch(model: nn.Module, thermal_mean: float = 128.0, thermal_std: float = 58.0) -> None:
    pm = getattr(model, "pixel_mean", None)
    ps = getattr(model, "pixel_std", None)
    if pm is None or ps is None:
        return
    if pm.numel() == 1 and ps.numel() == 1:
        return
    device = pm.device
    dtype = pm.dtype
    new_pm = torch.tensor([float(thermal_mean)], dtype=dtype, device=device).view(1, 1, 1)
    new_ps = torch.tensor([float(thermal_std)], dtype=dtype, device=device).view(1, 1, 1)
    model.register_buffer("pixel_mean", new_pm)
    model.register_buffer("pixel_std", new_ps)
