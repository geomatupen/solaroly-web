...
from ..dataops.photogrammetry_export import finalize_photogrammetry_export
_TILER_STATS = {}
...
async def api_test_run(
    ...
    embed_aligned_gps_for_photogrammetry: bool = Form(default=False),
    ...
    if embed_aligned_gps_for_photogrammetry and not (
        alignment_enabled and undistort_thermal and export_undistorted_images
    ):
        raise HTTPException(
            status_code=400,
            detail=(
                "Corrected photogrammetry GPS export requires LightGlue image alignment, automatic lens correction, "
    ...
    _write_result_status(
        ...
        photogrammetry_aligned_gps_requested=bool(embed_aligned_gps_for_photogrammetry),
    ...
    photogrammetry_export_summary: Optional[dict[str, Any]] = None
    if alignment_enabled:
        ...
        if embed_aligned_gps_for_photogrammetry:
            test_logger.info("UI:INFO:test: Embedding corrected horizontal GPS into photogrammetry export copies…")
            try:
                photogrammetry_export_summary = await asyncio.to_thread(
                    finalize_photogrammetry_export,
                ...
                _write_result_status(out_root, "failed", dataset=dataset, model=model_dir.name, error=str(exc))
                test_logger.error("UI:ERR:test: Photogrammetry export finalization failed: %s", exc)
                raise HTTPException(status_code=400, detail=f"Photogrammetry export finalization failed: {exc}") from exc
            test_logger.info(
                "UI:OK:test: Photogrammetry export ready: corrected GPS embedded in %s images; "
                "%s retained original GPS; geo.txt written.",
                int(photogrammetry_export_summary.get("corrected_gps_embedded") or 0),
                int(photogrammetry_export_summary.get("original_gps_retained") or 0),
            )
    if mosaic_enabled:
        try:
    ...
    try:
        ...
        metrics["photogrammetry_aligned_gps_exported"] = photogrammetry_export_summary is not None
        if photogrammetry_export_summary is not None:
            metrics["photogrammetry_export"] = photogrammetry_export_summary
    ...
    _write_result_status(
        ...
        photogrammetry_aligned_gps_exported=photogrammetry_export_summary is not None,
        photogrammetry_export=photogrammetry_export_summary,
        inference_source=source_mode,
        mosaic_created=mosaic_enabled,
        target_surface_height_m=float(target_surface_height_m),
        image_alignment_mode=alignment_mode,
