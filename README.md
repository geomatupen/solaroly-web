...
The Test tab has a matching right-side asset library for **Test datasets** and saved **Model results**. Both lists show separate display names and stable folder IDs, support display-name changes without moving their folders, and provide scoped delete actions. Opening a result switches to its full Results view, while selecting a test-dataset card makes it the active inference input.
