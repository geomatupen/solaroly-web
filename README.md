...
| LightGlue image alignment | `lightglue @ git+https://github.com/cvg/LightGlue.git@edb2b8...`, `kornia`, `torch`, `torchvision`, `opencv-python-headless` | Keep these when raw thermal-image GPS/orientation is not accurate enough and prediction GeoJSON or an approximate mosaic must use refined poses. Without LightGlue, leave **Align individual images before inference** disabled. | Selected per test run under **Test → Advanced**; CUDA is used when available and CPU remains supported. |
...
When **Align individual images before inference** and the photogrammetry export are both selected, the nested **Embed LightGlue-corrected GPS for WebODM** option is available. After alignment, it updates latitude/longitude only in aligned exported copies, leaves original dataset images and absolute altitude intact, and adds `geo.txt`, `camera_meta.json`, `image_alignment.json`, and `webodm_export.json` to the export folder. Images without a verified LightGlue correction retain their original GPS and are still listed in `geo.txt` when valid coordinates are available.
