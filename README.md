...
The Train tab's **Training data** asset panel has a dedicated uploader separate from test-data intake. It accepts one ZIP archive or a browser-selected folder using `train/` and `valid/` splits containing images plus COCO annotation JSON, matching YOLO `.txt` sidecars, or both. The uploader preserves this layout and registers the dataset when at least one supported annotation format validates. Its recorded backend compatibility determines whether it appears for Detectron, YOLO, or both in the Train selector. Uploaded datasets receive a stable ID and can be opened from the asset list for a validation summary. The selected dataset is revalidated immediately before each run, and its ID, name, path, and format are recorded in model metadata.

For projects stored on a mounted drive such as `/mnt/e`, the suggested upload destination is `/mnt/e/SolarolyProjects/Training_Data`. Set `PVRT_TRAINING_DATA_ROOT` to an absolute Windows, WSL, or server path to override this location.
